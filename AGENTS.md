...
|---------|-----------|-------|
| Backend tests | `./gradlew test` (monolith) or `./gradlew :services:core:test` (multi-module) | Uses H2 in-memory DB; no external services needed. `CalendarServiceClient` is always mocked. |
