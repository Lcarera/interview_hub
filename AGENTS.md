...
- Running the full application requires external secrets (`DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `JWT_SIGNING_SECRET`, `TOKEN_ENCRYPTION_KEY`) — see `CLAUDE.md` for the full list.
- Login requires a `@gm2dev.com` Google Workspace account (federated via TELUS Digital OneLogin).
